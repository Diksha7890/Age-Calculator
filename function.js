function submit() {
  ...
  // Validating  the data
  if (Bdydate == "" || Bdymonth == "" || Bdyyear == "") {
    showmsg("please provide  complete details");
  } else if (isNaN(Bdydate)) {
    showmsg("please enter valid date");
  } else if (Bdydate <= 0) {
    showmsg("Date cannot be zero or less than zero");
  } else if (Bdydate > months[Bdymonth - 1]) {
    showmsg("Please provide valid date with respect to month");
  } else if (isNaN(Bdymonth)) {
    ...
    curYear -= Bdyyear;
    showmsg(
      `Your age is ${curYear} years ${curMonth} months and ${curDate} days`
    );
  }
}
// showing current age on ui
